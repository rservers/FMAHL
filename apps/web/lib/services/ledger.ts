 ...
 */
export async function updateProviderBalance(providerId: string, balance: number): Promise<void> {
  await sql`
    UPDATE providers
    SET balance = ${balance}
    WHERE id = ${providerId}
 ...
 */
export async function calculateBalance(providerId: string): Promise<number> {
