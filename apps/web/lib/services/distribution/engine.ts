...
import type { DistributionResult, AssignmentDetail, SkippedProvider } from '../../types/distribution'
...
export async function distributeLead(
  ...
  try {
    ...
    const { startOrderPosition, competitionLevelIds: traversalLevelIds } =
      await getAndAdvanceStartLevel(lead.niche_id)
    ...
    let eligibleByLevel: Record<string, Array<{ id: string; provider_id: string; competition_level_id: string; subscription_id: string }>>
    try {
      const eligibleResult = await getEligibleSubscriptionsByLevel(leadId)
      eligibleByLevel = eligibleResult as any
    } catch (error) {
    ...
    const assignedProviderIds = new Set<string>() // Cross-level dedupe
    ...
    for (const levelInfo of traversalOrder) {
      ...
      const selectedProviders = await selectProvidersForLevel(
        levelInfo.levelId,
        levelInfo.maxRecipients,
        Array.from(assignedProviderIds),
        eligibleSubs.map(s => s.subscription_id)
      )
      ...
      for (const selected of selectedProviders) {
        ...
        try {
          ...
          const result = await retryWithBackoff(() =>
            createAssignmentWithCharge(
              leadId,
              selected.providerId,
              selected.subscriptionId,
              levelInfo.levelId,
              levelInfo.pricePerLeadCents
            )
          )
    ...
    // 7. Determine status
    let status: DistributionResult['status']
    if (assignmentsCreated === 0) {
