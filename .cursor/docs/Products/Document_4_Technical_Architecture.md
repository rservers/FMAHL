...
**Version:** 1.2 (LOCKED)
**Date:** January 1, 2026
**Status:** MVP Architecture (Locked for Build)
...
- **Integration**: Provider checkout (Stripe Checkout / PayPal Checkout) for deposits; provider APIs for refunds where applicable
- **Webhooks**: Stripe + PayPal webhooks for payment confirmation
...
- **Tracking**: Email event tracking (sent/delivered/opened/bounced) captured now (informational only; not used to auto-refund)
...
External Services:
┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐
│   Stripe / PayPal      │  │   Amazon SES           │  │   TrueNAS S3           │
...
CREATE TABLE providers (
  ...
  notification_preferences JSONB DEFAULT '{"new_lead": true, "bad_lead_global_decision": true, "low_balance": true}'::jsonb,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  deleted_at TIMESTAMPTZ
);
...
CREATE TABLE niches (
  ...
  next_start_level_order_position INTEGER NOT NULL DEFAULT 1, -- competition-level starting rotation pointer
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE leads (
  ...
  status VARCHAR(30) DEFAULT 'pending_confirmation' CHECK (status IN ('pending_confirmation', 'pending_approval', 'approved', 'distributed', 'rejected', 'bad_lead_global')),
...
CREATE TABLE audit_log (
  ...
  actor_role VARCHAR(20), -- captured at event time
  admin_only_memo TEXT,  -- internal-only notes
...
#### 14. email_events (NEW)
Captures email lifecycle events for **provider + end-user** emails (sent/delivered/opened/bounced).

**Policy:** Informational only; not used to auto-refund or auto-change lead state.
```sql
CREATE TABLE email_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  email_type VARCHAR(50) NOT NULL, -- e.g., lead_confirmation, lead_rejected, provider_new_lead
  recipient_email VARCHAR(255) NOT NULL,
  event_type VARCHAR(20) NOT NULL CHECK (event_type IN ('sent','delivered','opened','bounced')),
  provider VARCHAR(50), -- SES / SendGrid / etc (future)
  related_entity_type VARCHAR(50), -- leads, bad_lead_requests, etc
  related_entity_id UUID,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_email_events_recipient_email ON email_events(recipient_email);
CREATE INDEX idx_email_events_related_entity ON email_events(related_entity_type, related_entity_id);
...
### Filter Matching Contract (LOCKED)
Filter matching is the **core eligibility gate** and is evaluated in **application code**.

The `distribute_lead()` database function is responsible only for **assigning** a lead to a set of already-eligible subscriptions.

This prevents ambiguity and keeps filter logic testable and evolvable.


---

### Distribution Responsibilities (LOCKED)

To avoid ambiguity between SQL and application logic:

- **Rotation across competition levels (starting level per lead)** is handled by the **distribution job (application code)** in a single transaction that locks the niche row (`SELECT ... FOR UPDATE`) and updates `niches.next_start_level_order_position`.
- **Filter eligibility** is computed in **application code**.
- The database function `distribute_lead()` is intentionally **per-competition-level** and performs **assignment + ledger/balance/last_received_at updates** for **already-eligible subscriptions** within the specified level.

This means `distribute_lead()` correctly accepts `p_competition_level_id` (not `p_niche_id`), because the job iterates levels starting from the rotated starting position and calls `distribute_lead()` once per level with the eligible subscription set for that level.

**Implementation Note:** Some external suggestions may propose moving rotation logic into the SQL function (i.e., passing `p_niche_id`). That approach is valid in principle, but it conflicts with this MVP’s locked separation of concerns because it would require the DB function to: lock the niche row, iterate multiple levels, and also re-evaluate eligibility. It also often assumes different schema fields (e.g., a `subscriptions` table name or `leads.subscription_id`) that do not exist in this design.

For MVP, rotation + eligibility stay in the application job and `distribute_lead()` remains a per-level assignment primitive.
...
```sql
-- NOTE (LOCKED): This function assigns within a SINGLE competition level. Rotation across levels is handled by the distribution job (application code).
CREATE OR REPLACE FUNCTION distribute_lead(
  p_lead_id UUID,
  p_competition_level_id UUID,
  p_eligible_subscription_ids UUID[]
...
BEGIN
  ...
  FOR v_provider IN
    ...
    WHERE ps.competition_level_id = p_competition_level_id
      ...
      AND ps.id = ANY(p_eligible_subscription_ids)
      -- Filter matching is evaluated in application code (LOCKED contract)
...
**Approval Guard (LOCKED):** Admin can only approve a lead when `status = pending_approval` **and** `confirmed_at IS NOT NULL`.
POST   /api/v1/admin/leads/:id/reject   # Reject lead (optional notify_end_user toggle)
POST   /api/v1/admin/leads/:id/bad-lead # Mark as bad lead (global → sets status bad_lead_global)
...
POST   /api/v1/providers/me/deposit     # Create checkout session (stripe | paypal)
```
...
4. API validates request (Zod schema)
   ↓
5. Create lead record (status: pending_confirmation)
   ↓
6. Generate confirmation token (store hash + expiry)
   ↓
7. Queue confirmation email (BullMQ)
   ↓
8. End user clicks confirmation link
   ↓
9. API validates token, sets confirmed_at, status → pending_approval
   ↓
10. Admin receives notification (new lead pending approval)
```
...
5. Distribution job starts
   ↓
6. Read niche rotation pointer (next_start_level_order_position) **in a transaction** (SELECT ... FOR UPDATE)
   ↓
7. Start at that competition level and cascade forward (wrapping once)
   ↓
8. Update rotation pointer for next lead (same transaction)
   ↓
9. For each competition level (starting from rotated level):
   ↓
   a. Compute eligible subscriptions in application code (filters + business rules)
      - DB assignment function receives only eligible subscription IDs
   b. Sort by last_received_at (within-level least-recently-served)
...
**Gating Rule:** After a lead is `distributed`, only `bad_lead_global` (admin-only) or per-assignment refunds are allowed.
```
1. Admin marks lead as bad
   ↓
2. POST /api/v1/admin/leads/:id/bad-lead
   ↓
3. Update lead status (bad_lead_global)
   ↓
...
pg_dump -h localhost -U postgres -d findmeahotlead | \
  gzip > /backups/fmhl_$(date +%Y%m%d).sql.gz

# Upload to Wasabi
aws s3 cp /backups/fmhl_$(date +%Y%m%d).sql.gz \
  s3://fmhl-backup/db/ \
...
aws s3 sync s3://fmhl-uploads s3://fmhl-backup/files/ \
  --source-endpoint-url=https://truenas.local:9000 \
  --endpoint-url=https://s3.wasabisys.com
```
